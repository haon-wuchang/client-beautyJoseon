...
export default function Delivery() {
    ...
    const handleDelivery = () => {
        if (deliValidate()) {
            ...
            axios.post('http://3.36.132.223:9000/mypage/addDelivery', { 'id': id, 'deliData': deliData })
                .then(res => {
                    if (res.data.reslt === 1) {
                        getMyinfo();
                    }
...
const ChangeOriginDelivery = async() => {
    ...
    if(isChecked2){
        ...
        await axios.post('http://3.36.132.223:9000/mypage/updateMainDelivery', { deliData2, 'id': id })
        .then(res => {
            getMyinfo();
        })
        .catch(err => console.log(err));      
            
        await axios.post('http://3.36.132.223:9000/mypage/addDelivery',{ 'id': id, 'deliData': adf })
        .then(res => {
            getMyinfo();          
        })
        .catch(err => console.log(err));      
...
const deliveryDelete = () => {
    const id = localStorage.getItem('user_id');
    axios.post('http://3.36.132.223:9000/mypage/deleteDelivery', { 'id': id })
        .then(res => {
            getMyinfo();
        })
        .catch(err => console.log(err));
