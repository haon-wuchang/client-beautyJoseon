...
export default function UpdateMypage({ myinfo, births, Checked }) {
    ...
    const handleUpdateInfo = (colName, value) => {
        ...
        if (colName === 'pwd') {
            if (pwdValidate()) {
                axios.post('http://3.36.132.223:9000/mypage/updateInfo', { 'id': id, 'colName': colName, 'value': value })
                    .then(res => {
    ...
    const handleUpdateInfoAdd = () => {
        ...
        axios.post('http://3.36.132.223:9000/mypage/updateInfo', { 'id': id, 'colName': 'birth', 'value': est })
            .then(res => {
                if (res.data.result === 1) {
                    getMyinfo();
                } else {
    ...
    const deleteMyAllInfo = () => {
        ...
        if (handleDeleteInfo) {
            const id = localStorage.getItem('user_id');
             axios.post('http://3.36.132.223:9000/mypage/deleteUser', { 'id': id })
                .then(res => {                    
