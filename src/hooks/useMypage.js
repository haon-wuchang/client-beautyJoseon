...
const { isLoggedIn } = useContext(AuthContext);
    ...
    const getMyinfo = async () => {
        ...
        const result = await axios.post('http://3.36.132.223:9000/mypage/getMyinfo', { 'id': id });
    ...
    const getMyOrder = async () => {
        const id = localStorage.getItem('user_id');
        const result = await axios.post('http://3.36.132.223:9000/mypage/getMyOrder', { 'id': id });
    ...
    const getWishNumber = async () => {
        const id = localStorage.getItem('user_id');
        const result = await axios.post('http://3.36.132.223:9000/mypage/getWishNumber', { 'id': id });
        if (result.data.wish !== null && isLoggedIn === true) {
            const list = result.data.wish;
            // setWishList(list);
            const wishListData = await Promise.all(
                list.map(async (item) => {
                    const res = await axios.post('http://3.36.132.223:9000/mypage/getWishInfo', { pid: item });
                    return res.data;
                })
            );
    ...
    const getReview = async(type) => {
        const id = localStorage.getItem('user_id');
        const result = await axios.post('http://3.36.132.223:9000/mypage/getReview', { 'id': id ,'type':type});
    ...
    const getLastPid = () =>{
        axios.post('http://3.36.132.223:9000/uploads/getLastPid')
                .then(res => {
